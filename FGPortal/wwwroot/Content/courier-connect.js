(function (window, $) {
    ...
    var googleMapLib = function () {
        ...
        function createMap(mapOptions) {
            ...
            if (!courierConnect.helpers.isNullOrEmpty(mapElement)) {
                if (courierConnect.helpers.isNullOrEmpty(mapOptions)) {
                    mapOptions = {
                        zoom: 4.3,
        ...
        function ensureMapSetup() {
            if (courierConnect.helpers.isNullOrEmpty(_map)) {
                createMap();
            }
        }
        function setMarkers(markers) {
            ...
            for (var i = 0; i < markers.length; i++) {
                ...
                marker.addListener('click', function () {
                    if (!courierConnect.helpers.isNullOrEmpty(this.info.marker.state)) {
                        this.info.close();
                    ...
                    else if (!courierConnect.helpers.isNullOrEmpty(this.info.marker.url)) {
        ...
        function showMarkersByClasses(classes) {
            for (var i = 0; i < _markers.length; i++) {
                if (courierConnect.helpers.isIntersect(_markers[i].info.marker.classes.split(','), classes))
                    _markers[i].setVisible(true);
            }
        }
        function showMarkersByClassesForStateFilter(classes) {
            for (var i = 0; i < _markers.length; i++) {
                if (courierConnect.helpers.isIntersect(_markers[i].info.marker.classes.split(','), classes)
                    && courierConnect.helpers.isIntersectAny(_markers[i].info.marker.classes.split(','), _viewableStates)) {
                    _markers[i].setVisible(true);
        ...
        function zoomToFitStateMarkers(state) {
            var bounds = new google.maps.LatLngBounds();
            for (var i = 0; i < _markers.length; i++) {
                if (courierConnect.helpers.isIntersectAny(_markers[i].info.marker.classes.split(','), [ state ]))
    ...
    var aerisLib = function () {
        ...
        function init() {
            _apiID = courierConnect.helpers.isLocalhost() ? 'wgE96YE3scTQLKjnqiMsv' : 'GGAsOvzLvM7MkVml3X2m9';
            _apiSe = courierConnect.helpers.isLocalhost() ? 'tlyy22v5uBRBcm8lWeP0Y6ZISPLDVKGWXTJH9kYb' : 'fhQFx6yH6vNCwsE840LTlbo23KFtFHmT86B4CFD4';
		}
    ...
    window.courierConnect = new coreLib();
    window.courierConnect.helpers = new helpersLib();
    window.courierConnect.maps = new googleMapLib();
    window.courierConnect.aeris = new aerisLib();
})(window, $);